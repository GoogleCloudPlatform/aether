...
use aether::mir::{
    BinOp, Builder, Constant, ConstantValue, Function, Operand, Place, Program, Rvalue, SourceInfo,
...
fn test_dead_code_elimination_optimization() {
    ...
    let original_statement_count: usize = function
        .basic_blocks
        .values()
    ...
    let optimized_statement_count: usize = function
        .basic_blocks
        .values()
        .map(|block| block.statements.len())
        .sum();
    ...
    assert!(
        optimized_block_count < original_block_count
...
fn test_optimization_manager_default_pipeline() {
    ...
    program.functions.insert(
        "constant_test".to_string(),
        create_constant_folding_test_function(),
    );
    program.functions.insert(
        "dead_code_test".to_string(),
        create_dead_code_test_function(),
    );
    program
        .functions
        .insert("cse_test".to_string(), create_cse_test_function());
