...
fn test_external_function_analysis() {
    ...
    analyzer
        ...
        .expect("FFI analysis should succeed");
...
fn test_pointer_ownership_requirement() {
    ...
    assert!(errors
        .iter()
        .any(|e| e.to_string().contains("ownership info")));
}
...
fn test_calling_convention_support() {
    ...
    for convention in conventions {
        ...
        analyzer
            .analyze_program(&program)
            .unwrap_or_else(|_| panic!("Should handle {:?} calling convention", convention));
    }
}
...
fn test_complex_type_mapping() {
    ...
    let array_type = type_checker
        .ast_type_to_type(&TypeSpecifier::Array {
