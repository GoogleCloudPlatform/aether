...
(DEFINE_MODULE auto_fix
  (DEFINE_FUNCTION
    ...
    if let Some(error) = result.error() {
        ...
        assert!(
            error_msg.to_lowercase().contains("declare")
                || error_msg.to_lowercase().contains("define")
                || error_msg.to_lowercase().contains("variable"),
            "Error should suggest variable declaration: {}",
            error_msg
        );

        // Should provide example fix
        assert!(
            error_msg.contains("DECLARE_VARIABLE")
                || error_msg.contains("example")
                || error_msg.contains("try:"),
            "Error should provide example fix: {}",
            error_msg
        );
    }
}
  ...
  (DEFINE_FUNCTION
    ...
    if result.is_success() {
        ...
        if let Some(error) = result.error() {
            let error_msg = format!("{}", error);
            assert!(
                error_msg.contains("partial")
                    || error_msg.contains("working")
                    || error_msg.contains("successful"),
                "Error should mention partial compilation: {}",
                error_msg
            );
        }
    }
...
(DEFINE_MODULE llm_friendly
  (DEFINE_FUNCTION
    ...
    if let Some(error) = result.error() {
        ...
        assert!(
            error_msg.contains("type") || error_msg.contains("TYPE"),
        ...
        assert!(
            ...
            "Error should provide context about array access: {}",
            error_msg
        );

        // Should be structured for LLM consumption
        assert!(
            error_msg.contains("{") || error_msg.contains("Error:") || error_msg.contains("Code:"),
...
(DEFINE_MODULE intent_mismatch_error
  (DEFINE_FUNCTION
    ...
    assert!(
        has_llm_friendly_message,
        "Should have LLM-friendly intent mismatch message"
    );
}
...
(DEFINE_MODULE error_recovery
  ...
  (DEFINE_FUNCTION
    ...
    if let Some(error) = result.error() {
        ...
        assert!(
            mentions_recovery || mentions_progress,
            "Error should mention recovery or progress: {}",
            error_msg
        );
    }
}
...
(DEFINE_MODULE cascading_errors
  (DEFINE_FUNCTION
    ...
    if let Some(error) = result.error() {
        ...
        assert!(
            error_count <= 5,
            "Should not have excessive error messages (found {}): {}",
            error_count,
            error_msg
        );
        ...
        assert!(
            error_msg.contains("undefined") || error_msg.contains("undefined_var"),
            "Should identify primary error: {}",
            error_msg
        );
    }
}
...
(DEFINE_MODULE contextual_errors
  (DEFINE_FUNCTION
    ...
    if let Some(error) = result.error() {
        ...
        assert!(
            error_msg.contains("intent") || error_msg.contains("convert"),
            "Error should reference function intent: {}",
            error_msg
        );
