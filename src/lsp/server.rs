    ...
    async fn hover(&self, params: HoverParams) -> Result<Option<Hover>> {
        ...
        if let Some(doc_state) = state.documents.get(&uri) {
            if let Some(symbol_table) = &doc_state.symbol_table {
                ...
                let identifier_at_cursor =
                    Self::find_identifier_at_position(&doc_state.text, position);
...
impl Backend {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            match module_result {
                    ...
                    match analyzer.analyze_program(&program) {
                        ...
                        Err(errors) => {
                            ...
                            let diagnostics = errors
                                .into_iter()
                                .map(|err| Self::semantic_error_to_diagnostic_static(&err))
                                .collect();
                ...
                Err(err) => {
                    ...
                    rt.block_on(async {
                        client
                            .publish_diagnostics(uri_clone, vec![diagnostic], None)
                            .await;
                    });
                }
            }
        ...
        match error {
            CompilerError::Parser { source } => {
                match source {
                    ...
                    crate::error::ParserError::SyntaxError {
                        message,
                        location,
                        suggestion: _,
                    } => Diagnostic {
    ...
    fn semantic_error_to_diagnostic_static(error: &crate::error::SemanticError) -> Diagnostic {
        let (message, location) = match error {
            crate::error::SemanticError::UndefinedSymbol { symbol, location } => {
                (format!("Undefined symbol: {}", symbol), location)
            }
            crate::error::SemanticError::TypeMismatch {
                expected,
                found,
                location,
            } => (
            ...
            crate::error::SemanticError::AssignToImmutable { variable, location } => (
                format!("Cannot assign to immutable variable: {}", variable),
                location,
            ),
            crate::error::SemanticError::UseBeforeInitialization { variable, location } => (
                format!("Variable used before initialization: {}", variable),
                location,
            ),
            crate::error::SemanticError::UseAfterMove { variable, location } => {
                (format!("Variable used after move: {}", variable), location)
            }
