                ...
                ast::Statement::VariableDeclaration {
                    ...
                    initial_value: Some(Box::new(ast::Expression::FunctionCall {
                        call: ast::FunctionCall {
                            ...
                            arguments: vec![
                                ast::Argument {
                                    parameter_name: Identifier::new("val".to_string(), SourceLocation::unknown()),
                    ...
                    initial_value: Some(Box::new(ast::Expression::FunctionCall {
                        call: ast::FunctionCall {
                            ...
                            arguments: vec![
                                ast::Argument {
                                    parameter_name: Identifier::new("val".to_string(), SourceLocation::unknown()),
                                    value: Box::new(ast::Expression::FloatLiteral { value: 3.14, source_location: SourceLocation::unknown() }),
                                    source_location: SourceLocation::unknown(),
                                }
                            ],
                            variadic_arguments: vec![],
...
fn test_monomorphization_simple() {
    ...
    semantic_analyzer.analyze_program(&ast_program).expect("Semantic analysis should pass");
    ...
    let mut mir_program = lowering_context.lower_program(&ast_program)
    ...
    assert!(!mir_program.functions.contains_key("test_module.identity_Integer"));
    assert!(!mir_program.functions.contains_key("test_module.identity_Float"));
    ...
    assert_eq!(int_identity.return_type, Type::primitive(PrimitiveType::Integer));
    assert_eq!(int_identity.parameters[0].ty, Type::primitive(PrimitiveType::Integer));

    let float_identity = &mir_program.functions[&mangled_float_name];
    assert_eq!(float_identity.name, mangled_float_name);
    assert_eq!(float_identity.return_type, Type::primitive(PrimitiveType::Float));
    assert_eq!(float_identity.parameters[0].ty, Type::primitive(PrimitiveType::Float));

    // 3. Check that calls in 'main' are updated to mangled names
    let main_func = mir_program.functions.get("main").expect("main function not found");
    let mut found_int_call = false;
    let mut found_float_call = false;

    for (_block_id, block) in &main_func.basic_blocks {
        for stmt in &block.statements {
            if let Statement::Assign { rvalue: Rvalue::Call { func: Operand::Constant(Constant { value: ConstantValue::String(called_name), .. }), .. }, .. } = stmt {
    ...
    assert!(found_int_call, "Call to identity<Int> not monomorphized correctly");
    assert!(found_float_call, "Call to identity<Float> not monomorphized correctly");
