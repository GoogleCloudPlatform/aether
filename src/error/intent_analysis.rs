...
impl IntentAnalyzer {
    ...
    pub fn analyze_function(&mut self, function: &Function) -> IntentAnalysisResult {
        ...
        let stated_intent =
            function
                .intent
                .as_ref()
                .map(|intent_str| crate::semantic::metadata::IntentSpec {
        ...
        // Cache result
        self.cache
            .insert(function.name.name.clone(), result.clone());
    ...
    fn find_mismatches(
        ...
        if let Some(intent) = stated_intent {
            ...
            if let Some(mismatch) =
                self.check_primary_intent(&intent.primary_intent, &inferred.primary_action)
            {
            ...
            if false {
                // TODO: Check pure behavior when metadata available
            ...
            if intent.failure_modes.is_empty()
                && matches!(inferred.error_handling, ErrorHandlingStyle::None)
            {
                mismatches.push(IntentMismatch {
    ...
    fn calculate_confidence(
        ...
        let complexity_penalty = if behavior.side_effects.len() > 3 {
            0.1
        } else {
            0.0
        };
    ...
    fn analyze_statement(&self, stmt: &Statement, context: &mut BehaviorContext) {
        if let Statement::Assignment { target, value, .. } = stmt {
            ...
            if has_side_effects(value) {
                context
                    .side_effects
                    .push("Variable modification".to_string());
            }
        }
    }
}
...
impl StatementAnalyzer for FunctionCallAnalyzer {
    fn analyze_statement(&self, stmt: &Statement, context: &mut BehaviorContext) {
        if let Statement::FunctionCall { call, .. } = stmt {
            ...
            context
                .actions
                .push(format!("Calls function '{}'", func_name));

            // Check for known side-effect functions
            if is_side_effect_function(func_name) {
                context
                    .side_effects
                    .push(format!("Calls {} (has side effects)", func_name));
            }
            ...
            if is_resource_function(func_name) {
                context
                    .resource_usage
                    .push(format!("Uses {} resources", func_name));
            }
        }
    }
}
    ...
    fn analyze_statement(&self, stmt: &Statement, context: &mut BehaviorContext) {
        match stmt {
            Statement::If { .. } => {
                ...
                context
                    .actions
                    .push("Makes conditional decisions".to_string());
            }
    ...
    fn analyze_statement(&self, stmt: &Statement, context: &mut BehaviorContext) {
        match stmt {
            Statement::TryBlock { .. } => {
                ...
                context
                    .actions
                    .push("Handles errors with try-catch".to_string());
            }
            Statement::Throw { .. } => {
                context.has_error_handling = true;
...
fn is_side_effect_function(name: &str) -> bool {
    matches!(
        name,
        "printf" | "write" | "append" | "delete" | "update" | "send"
    )
}
...
mod tests {
    use super::*;
    use crate::ast::{
        Block, FunctionCall, FunctionReference, Identifier, PrimitiveType, TypeSpecifier,
    };
