...
impl RegistryClient {
    ...
    pub fn get_package_info(&mut self, package_name: &str) -> Result<PackageInfo, SemanticError> {
        ...
        let url = format!(
            "{}/api/v1/packages/{}",
            self.config.default_url, package_name
        );
        ...
        // Cache the result
        self.cache
            .cache_package_metadata(package_name, metadata.clone());
    ...
    pub fn get_package_versions(
        &mut self,
        package_name: &str,
    ) -> Result<Vec<Version>, SemanticError> {
        // Check cache first
        ...
        let response: ApiResponse<Vec<VersionMetadata>> =
            self.client.get(&url, &self.auth_tokens)?;
        ...
        let versions: Vec<Version> = version_metadata
            .iter()
            .filter(|v| !v.yanked)
        ...
        // Cache the result
        self.cache
            .cache_version_list(package_name, versions.clone());
    ...
    pub fn get_package_manifest(
        ...
        package_name: &str,
        version: &Version,
    ) -> Result<PackageManifest, SemanticError> {
        let url = format!(
            "{}/api/v1/packages/{}/{}/manifest",
    ...
    pub fn search_packages(
    ...
    ) -> Result<Vec<PackageInfo>, SemanticError> {
        let cache_key = format!(
            "{}:{:?}:{:?}",
            options.query, options.limit, options.sort_by
        );
        ...
        let response: ApiResponse<Vec<PackageMetadata>> =
            self.client.get(&url, &self.auth_tokens)?;
        ...
        let package_infos: Vec<PackageInfo> = search_results
            .iter()
        ...
        // Cache the result
        self.cache
            .cache_search_results(&cache_key, package_infos.clone());
    ...
    pub fn publish_package(
        ...
        // Read archive data
        let archive_data = std::fs::read(archive_path).map_err(|e| SemanticError::Internal {
        ...
        let response: ApiResponse<()> =
            self.client
                .post(&url, &publish_request, &self.auth_tokens)?;
    ...
    fn get_download_url(
        ...
        if !response.success {
            return Err(SemanticError::Internal {
                message: response
                    .error
                    .unwrap_or_else(|| "Failed to get download URL".to_string()),
            });
        }
        ...
        // Cache the result
        self.cache
            .cache_download_url(&cache_key, download_url.clone());
...
impl HttpClient {
    fn get<T>(
        &self,
        _url: &str,
        _auth_tokens: &HashMap<String, String>,
    ) -> Result<ApiResponse<T>, SemanticError>
    where
    ...
    fn post<T, U>(
        ...
        _body: &T,
        _auth_tokens: &HashMap<String, String>,
    ) -> Result<ApiResponse<U>, SemanticError>
    where
        T: Serialize,
...
impl RequestCache {
    ...
    fn cache_package_metadata(&mut self, package_name: &str, metadata: PackageMetadata) {
        ...
        self.package_metadata
            .insert(package_name.to_string(), cached);
    }
...
mod tests {
    ...
    fn test_registry_config_creation() {
        ...
        assert_eq!(
            registry_config.default_url,
            "https://registry.aetherscript.org"
        );
