...
pub fn create_collections_module() -> Module {
    ...
    Module {
        ...
        intent: Some(
            "Provides efficient array and map data structures with utility functions".to_string(),
        ),
        imports: vec![],
        exports: vec![
...
mod tests {
    ...
    fn test_collections_module_creation() {
        ...
        assert!(module
            .external_functions
            .iter()
            .any(|f| f.name.name == "array_create"));
        assert!(module
            .external_functions
            .iter()
            .any(|f| f.name.name == "array_push"));
        assert!(module
            .external_functions
            .iter()
            .any(|f| f.name.name == "array_pop"));
        assert!(module
            .function_definitions
            .iter()
            .any(|f| f.name.name == "array_sort"));
        assert!(module
            .function_definitions
            .iter()
            .any(|f| f.name.name == "array_map"));

        // Check map operations
        assert!(module
            .external_functions
            .iter()
            .any(|f| f.name.name == "map_create"));
        assert!(module
            .external_functions
            .iter()
            .any(|f| f.name.name == "map_insert"));
        assert!(module
            .external_functions
            .iter()
            .any(|f| f.name.name == "map_get"));
        assert!(module
            .function_definitions
            .iter()
            .any(|f| f.name.name == "map_keys"));

        // Check exports
        assert!(module.exports.iter().any(
            |e| matches!(e, ExportStatement::Function { name, .. } if name.name == "array_create")
        ));
        assert!(module.exports.iter().any(
            |e| matches!(e, ExportStatement::Function { name, .. } if name.name == "map_create")
        ));
        assert!(module.exports.iter().any(
            |e| matches!(e, ExportStatement::Function { name, .. } if name.name == "array_sort")
        ));
    }
