...
pub fn create_memory_module() -> Module {
    ...
    Module {
        ...
        intent: Some(
            "Provides low-level memory management operations and allocation utilities".to_string(),
        ),
        imports: vec![],
        exports: vec![
...
mod tests {
    ...
    fn test_memory_module_creation() {
        ...
        assert!(module
            .external_functions
            .iter()
            .any(|f| f.name.name == "allocate"));
        assert!(module
            .external_functions
            .iter()
            .any(|f| f.name.name == "deallocate"));
        assert!(module
            .external_functions
            .iter()
            .any(|f| f.name.name == "reallocate"));
        assert!(module
            .external_functions
            .iter()
            .any(|f| f.name.name == "copy_memory"));

        // Check high-level functions
        assert!(module
            .function_definitions
            .iter()
            .any(|f| f.name.name == "create_buffer"));
        assert!(module
            .function_definitions
            .iter()
            .any(|f| f.name.name == "create_memory_pool"));

        // Check exports
        assert!(module.exports.iter().any(
            |e| matches!(e, ExportStatement::Function { name, .. } if name.name == "allocate")
        ));
    }
