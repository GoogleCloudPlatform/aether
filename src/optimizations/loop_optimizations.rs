...
use crate::mir::{BasicBlock, BinOp, Function, Operand, Place, Rvalue, Statement, Terminator};
...
impl LoopOptimizationPass {
    ...
    fn build_dominance_info(&mut self, function: &Function) -> Result<(), SemanticError> {
        ...
        for &block in &blocks {
            ...
            if let Some(dominators) = dom.get(&block) {
                // Remove block itself
                let mut strict_dom: Vec<usize> = dominators
                    .iter()
                    .filter(|&&x| x != block)
                    .copied()
                    .collect();
    ...
    fn find_natural_loop(
        ...
        head: usize,
        tail: usize,
    ) -> Result<HashSet<usize>, SemanticError> {
    ...
    fn find_preheader(
        ...
        let external_preds: Vec<usize> = predecessors
            .into_iter()
                ...
                if i != j {
                    ...
                    if loop_j.blocks.len() > loop_i.blocks.len()
                        && loop_i
                            .blocks
                            .iter()
                            .all(|block| loop_j.blocks.contains(block))
                    {
                        // This is a potential parent
                        if parent_loop.is_none()
                            || self
                                .loops
                                .get(parent_loop.unwrap())
                                .map(|l: &LoopInfo| l.blocks.len())
                                .unwrap()
                                > loop_j.blocks.len()
                        {
                            parent_loop = Some(j);
    ...
    fn analyze_loop_invariants(&mut self, function: &Function) -> Result<(), SemanticError> {
        ...
        for (_loop_idx, loop_info) in self.loops.iter().enumerate() {
                    ...
                    for (stmt_idx, statement) in block.statements.iter().enumerate() {
                        if self.is_loop_invariant(statement, loop_info, function)? {
                            let invariant_stmt = InvariantStatement {
                                ...
                                safe_to_hoist: self
                                    .is_safe_to_hoist(statement, loop_info, function)?,
            ...
            self.invariant_analysis
                ...
                .insert(loop_info.header, invariant_statements);
            self.invariant_analysis
                .invariant_variables
                .insert(loop_info.header, invariant_variables);
        }
    ...
    fn is_rvalue_loop_invariant(
        ...
        match rvalue {
            ...
            Rvalue::BinaryOp { left, right, .. } => Ok(self
                .is_operand_loop_invariant(left, loop_info)?
    ...
    fn analyze_induction_variables(&mut self, function: &Function) -> Result<(), SemanticError> {
        ...
        for loop_info in &self.loops {
            ...
            self.find_derived_induction_variables(
                loop_info,
                function,
                &basic_ivs,
                &mut derived_ivs,
            ...
            self.induction_analysis
                .basic_induction_vars
                .insert(loop_info.header, basic_ivs);
            self.induction_analysis
                .derived_induction_vars
                .insert(loop_info.header, derived_ivs);
        }
                                    ...
                                    if let Operand::Constant(constant) = right {
                                        if let Some(step) = self.extract_integer_constant(constant)
                                        {
                                            let basic_iv = BasicInductionVar {
                                                ...
                                                initial_value: Operand::Constant(
                                                    crate::mir::Constant {
                                                        ty: function
                                                            .locals
                                                            .get(&place.local)
                                                        ...
                                                        value: crate::mir::ConstantValue::Integer(
                                                            0,
                                                        ),
                                                    },
                                                ),
            ...
            if let Some(block) = function.basic_blocks.get(&(block_id as u32)) {
                for statement in &block.statements {
                    if let Statement::Assign { place, rvalue, .. } = statement {
                        ...
                        if let Some(derived_iv) =
                            self.match_derived_iv_pattern(place, rvalue, basic_ivs)
                        {
    ...
    fn match_derived_iv_pattern(
        ...
        match rvalue {
            Rvalue::BinaryOp {
                ...
                // Pattern: j = i * c
                if let (Operand::Move(var) | Operand::Copy(var), Operand::Constant(constant)) =
                    (left, right)
                {
    ...
    fn analyze_data_dependencies(&mut self, function: &Function) -> Result<(), SemanticError> {
        ...
        let loops_data: Vec<(usize, Vec<usize>, LoopInfo)> = self
            .loops
            .iter()
            .map(|loop_info| {
                (
                    loop_info.header,
                    loop_info.blocks.iter().copied().collect(),
                    loop_info.clone(),
                )
            })
            .collect();
        ...
        for (header, blocks, loop_info) in loops_data {
            ...
            for block_id in blocks {
                if let Some(block) = function.basic_blocks.get(&(block_id as u32)) {
                    self.analyze_block_dependencies(
                        block,
                        block_id,
                        &loop_info,
                        &mut loop_carried_deps,
                    )?;
                }
            }
            ...
            self.dependence_analysis
                .loop_carried_deps
                .insert(header, loop_carried_deps);
        }
    ...
    fn operand_uses_place(&self, operand: &Operand, place: &Place) -> bool {
        match operand {
            Operand::Move(op_place) | Operand::Copy(op_place) => op_place.local == place.local,
    ...
    /// Apply loop invariant code motion
    fn apply_loop_invariant_code_motion(
        &mut self,
        ...
        for loop_info in &self.loops {
            if let Some(invariant_statements) = self
                .invariant_analysis
                .invariant_statements
    ...
    fn hoist_statement(
        ...
        eprintln!(
            "Would hoist statement from block {} (index {}) out of loop {}",
    ...
    fn apply_strength_reduction_to_iv(
        ...
        // For now, just report what would be done
        eprintln!(
            "Would apply strength reduction to derived IV in loop {}",
            loop_info.header
        );
        Ok(false)
    }
