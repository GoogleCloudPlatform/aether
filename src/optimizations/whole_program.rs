...
use crate::mir::{BasicBlock, Constant, Function, Operand, Program, Rvalue, Statement, Terminator};
...
impl WholeProgramOptimizationPass {
    ...
    pub fn analyze_program(
        &mut self,
        program: &Program,
    ) -> Result<HashMap<String, FunctionAnalysis>, SemanticError> {
        // Build call graph
    ...
    fn build_call_graph(&mut self, program: &Program) -> Result<(), SemanticError> {
        ...
        for (caller_name, function) in &program.functions {
            ...
            self.call_graph
                .calls
                .insert(caller_name.clone(), calls.clone());
