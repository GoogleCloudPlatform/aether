...
impl InterproceduralAnalysisPass {
    ...
    pub fn analyze_program(
        &mut self,
        program: &Program,
    ) -> Result<HashMap<String, FunctionSummary>, SemanticError> {
    ...
    fn build_call_graph(&mut self, program: &Program) -> Result<(), SemanticError> {
        ...
        for function_name in program.functions.keys() {
            self.call_graph
                .callees
                .insert(function_name.clone(), HashSet::new());
            self.call_graph
                .callers
                .insert(function_name.clone(), HashSet::new());
        }
        ...
        match statement {
            Statement::Assign { place, rvalue, .. } => {
                ...
                if self.is_global_place(place) {
                    ...
                    summary
                        .modifies_globals
                        .insert(self.extract_global_name(place));
                }
    ...
    fn analyze_escapes(
        ...
        for (function_name, function) in &program.functions {
            ...
            self.escape_analysis
                .escaping_variables
                .insert(function_name.clone(), escaping);
            self.escape_analysis
                .local_variables
                .insert(function_name.clone(), local);
            self.escape_analysis
                .passed_variables
                .insert(function_name.clone(), passed);
            self.escape_analysis
                .returned_variables
                .insert(function_name.clone(), returned);
        }
    ...
    fn apply_global_constant_propagation(
        &self,
        program: &mut Program,
    ) -> Result<bool, SemanticError> {
        let mut changed = false;
    ...
    fn eliminate_dead_functions(
        ...
        for function_name in program.functions.keys() {
            if function_name != "main" && !program.external_functions.contains_key(function_name) {
        ...
        if removed_count > 0 {
            eprintln!(
                "Eliminated {} dead functions via interprocedural analysis",
                removed_count
            );
        }
    ...
    fn optimize_pure_calls_in_function(
        &self,
