...
impl Parser {
    ...
    fn initialize_keywords(&mut self) {
        let keywords = [
            ...
            (
                ...
                KeywordType::ExpressionIntegerDivide,
            ),
            ...
            (
                ...
                KeywordType::PerformanceExpectation,
            ),
                    ...
                    match self.keywords.get(keyword) {
                                    ...
                                    if matches!(content_token.token_type, TokenType::LeftParen) {
                                        match self.parse_module_content_item() {
                                            Ok(content_item) => match content_item {
                                                ModuleContent::Import(import) => {
                                                    imports.push(import)
                                                }
                                                ModuleContent::Export(export) => {
                                                    exports.push(export)
                                                }
                                                ModuleContent::TypeDefinition(type_def) => {
                                                    type_definitions.push(type_def);
                                                }
                                                ModuleContent::ConstantDeclaration(const_decl) => {
                                                    constant_declarations.push(const_decl)
                                                }
                                            ...
                                            Err(error) => {
                                                ...
                                                eprintln!(
                                                    "Parser: Error parsing module content: {:?}",
                                                    error
                                                );
                                                self.record_error(error);
                                                self.synchronize();
                                        ...
                                        let err = ParserError::UnexpectedToken {
                                            ...
                                            expected: "left parenthesis for content item"
                                                .to_string(),
                        ...
                        _ => {
                            return Err(ParserError::UnexpectedToken {
                                found: keyword.clone(),
                                expected: "module field keyword (NAME, INTENT, CONTENT)"
                                    .to_string(),
    ...
    fn parse_module_content_item(&mut self) -> Result<ModuleContent, ParserError> {
        ...
        match &keyword_token.token_type {
            TokenType::Keyword(keyword) => match self.keywords.get(keyword) {
                ...
                match self.keywords.get(&keyword_clone) {
                    ...
                    _ => {
                        return Err(ParserError::UnexpectedToken {
                            found: keyword_clone,
                            expected:
                                "export keyword (EXPORTS_FUNCTION, EXPORTS_TYPE, EXPORTS_CONSTANT)"
                                    .to_string(),
    ...
    fn parse_type_definition(&mut self) -> Result<TypeDefinition, ParserError> {
        ...
        eprintln!(
            "Parser: Entering parse_type_definition at position {}",
            self.position
        );

        // Check which type definition keyword we have
        let type_def_keyword = self
            .current_token()
            .ok_or_else(|| ParserError::UnexpectedEof {
                expected: "type definition keyword".to_string(),
            })?;
        eprintln!(
            "Parser: Type def keyword: {:?}",
            type_def_keyword.token_type
        );
    ...
    fn parse_structured_type(
        ...
        eprintln!(
            "Parser: Creating struct '{}' with {} fields",
            name.name,
            fields.len()
        );
                ...
                TokenType::Keyword(keyword) => {
                    match self.keywords.get(keyword) {
                        ...
                        _ => {
                            return Err(ParserError::UnexpectedToken {
                                found: keyword.clone(),
                                expected: "constant field keyword (NAME, TYPE, VALUE, INTENT)"
                                    .to_string(),
                                location: field_keyword.location.clone(),
                            });
                        }
                            ...
                            let convention_str = {
                                ...
                                let result = match &token.token_type {
                                    ...
                                    _ => {
                                        return Err(ParserError::UnexpectedToken {
                                            found: format!("{:?}", token.token_type),
                                            expected: "calling convention (string or identifier)"
                                                .to_string(),
                                            location: token.location.clone(),
                                        });
                                    }
                ...
                TokenType::Keyword(keyword) => {
                    match self.keywords.get(keyword) {
                        ...
                        Some(KeywordType::Precondition) => {
                            ...
                            metadata
                                .preconditions
                                .push(self.parse_contract_assertion()?);
                        }
                        ...
                        Some(KeywordType::PerformanceExpectation) => {
                            ...
                            metadata.performance_expectation =
                                Some(self.parse_performance_expectation()?);
                        }
                        Some(KeywordType::ComplexityExpectation) => {
                            self.advance(); // consume COMPLEXITY_EXPECTATION
                            metadata.complexity_expectation =
                                Some(self.parse_complexity_expectation()?);
                        }
                        Some(KeywordType::ThreadSafe) => {
                            self.advance(); // consume THREAD_SAFE
    ...
    fn parse_ownership_annotation(
        &mut self,
    ) -> Result<(Option<OwnershipKind>, Option<SourceLocation>), ParserError> {
            ...
            TokenType::Keyword(keyword) => {
                match self.keywords.get(keyword) {
                    ...
                    Some(KeywordType::FunctionType) => {
                        ...
                        let return_type =
                            if let TokenType::Keyword(keyword) = &returns_token.token_type {
        ...
        let result = if let Some(keyword) = keyword_str {
            ...
            match &keyword_token.token_type {
                TokenType::Identifier(name) => {
                    ...
                    let variant_name =
                        Identifier::new(name.clone(), keyword_token.location.clone());
                        ...
                        Some(KeywordType::Name) => {
                            ...
                            name = Some(match self.current_token() {
                                Some(token) => match &token.token_type {
                                    ...
                                    TokenType::Identifier(_) => self.consume_identifier()?,
                                    _ => {
                                        return Err(ParserError::UnexpectedToken {
                                            found: format!("{:?}", token.token_type),
                ...
                TokenType::Keyword(keyword) => {
                    match self.keywords.get(keyword) {
                        ...
                        _ => {
                            return Err(ParserError::UnexpectedToken {
                                found: keyword.clone(),
                                expected: "assignment field (TARGET_VARIABLE or SOURCE_EXPRESSION)"
                                    .to_string(),
                                location: field_keyword.location.clone(),
                            })
                        }
            ...
            match &keyword_token.token_type {
                TokenType::Keyword(keyword) => match self.keywords.get(keyword) {
                    ...
                    _ => {
                        return Err(ParserError::UnexpectedToken {
                            found: keyword.clone(),
                            expected: "loop field keyword (COUNTER, FROM, TO, STEP, DO)"
                                .to_string(),
                            location: keyword_token.location.clone(),
                        });
                    }
                ...
                let (nested_pattern, binding) = if let Some(token) = self.current_token() {
                    match &token.token_type {
                        ...
                        TokenType::Identifier(binding_name) => {
                            ...
                            let binding_id =
                                Identifier::new(binding_name.clone(), token.location.clone());
...
mod tests {
    ...
    fn test_keyword_mapping() {
        ...
        assert_eq!(
            parser.keywords.get("DEFINE_MODULE"),
            Some(&KeywordType::DefineModule)
        );
    }
}
...
impl Parser {
    ...
    ///   (BODY ...))
    fn parse_resource_scope(
        &mut self,
        location: SourceLocation,
    ) -> Result<resource::ResourceScope, ParserError> {
                ...
                Some(KeywordType::ResourceBinding) => {
                    if let Some(token) = self.current_token() {
                        match &token.token_type {
                            TokenType::Identifier(name) => {
                                binding =
                                    Some(Identifier::new(name.clone(), token.location.clone()));
                                self.advance();
                            }
                            _ => {
                                return Err(ParserError::UnexpectedToken {
